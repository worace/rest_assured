class ShelterAvailability
  def self.check_shelters
    ...
    Shelter.shelters_needing_checkin.each do |shelter|
      ...
      client.messages.create(from: TWILIO_NUMBER, to: HORACE_NUMBER, body: "Automated Check In message from Rest Assured -- Please respond with bed availability for: #{shelter.name}")
    end
  end
end
